...
typedef struct __line {
    ...
    struct{
        uint32_t tag:20;
        uint32_t lru:12; //we only really need 3 bits to represent the lru in a set, the remaining 9 will likely be used in the next labs to implement state.
    };
} cache_line_t;

typedef std::array<cache_line_t, 8> set_t;
std::array<set_t,128>* cache;

void pr_arr(const std::array<set_t, 128>& arr)
{
    for(auto i : *cache) {
      for(auto j : i) {
          for(auto k : j.line) {
...
{
  cache_addr_t addr;
  addr.memory_addr = 0xffffffff;
  
  std::cout << addr.byte_offset << " -- " << addr.set_addr << " -- " << addr.tag << std::endl;
  
  cache = new std::array<set_t, 128>;
  cache_line_t line_tmp;
  line_tmp.line = {97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97};
  
  for(auto& set : *cache) {
      for(auto& line : set) {
          for(auto& i : line.line) {
              i = 97;
          }
      }
  }
...
int main()
{
  ...
  std::unique_ptr<std::array<set_t,128>> cache(new std::array<set_t, 128>);
  cache_line_t line_tmp;
  line_tmp.line = {97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97,97, 97};
